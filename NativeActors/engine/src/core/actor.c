...
const INT16 a_grav = 1;
const INT16 a_max_fall = 32;

void actor_move_bounce_x(actor_t *actor) BANKED {

    INT16 *mov = &script_memory[actor->varref[0] ];
    ...
    if( targ != newpos ){ 
        script_memory[actor->varref[0] ] = -(*mov); // bounce
        UBYTE new_dir = -(*mov) > 0 ? DIR_LEFT : DIR_RIGHT;
...
// TODO: ensure position is moved up to edge to help with walking collisions
void actor_move_bounce_y(actor_t *actor) BANKED {

    INT16 *mov = &script_memory[actor->varref[1] ];
    ...
    INT16 newpos = check_collision_in_direction(actor->pos.x, actor->pos.y, &actor->bounds, targ, dir);
    if( targ != newpos ){ 
        script_memory[actor->varref[1] ] = -(*mov); // bounce
    } else {
        actor->pos.y = newpos;
    }
}

void actor_move_stop_y(actor_t *actor) BANKED {
    ...
    INT16 newpos = check_collision_in_direction(actor->pos.x, actor->pos.y, &actor->bounds, targ, dir);
    if( targ != newpos ){ 
        script_memory[actor->varref[1] ] = 0; // bounce
    }
    actor->pos.y = newpos;
}

void actor_move_jump_y(actor_t *actor) BANKED {
    ...
    if( targ != newpos ){ 
        if(dir == CHECK_DIR_UP){
            ...
            script_memory[actor->varref[1] ] = -a_max_fall;
        }
    }
    actor->pos.y = newpos;
}

/*void actor_walker(actor_t *actor) BANKED {

    INT16 *movx = &script_memory[actor->varref[0] ];
    INT16 *movy = &script_memory[actor->varref[1] ];
    UBYTE dir = *mov < 0 ? CHECK_DIR_UP : CHECK_DIR_DOWN;
...
void actor_apply_gravity(actor_t *actor) BANKED {
    INT16 *yvel = &script_memory[actor->varref[1] ];
    script_memory[actor->varref[1] ] = MIN( (*yvel + a_grav), a_max_fall);
}

void actors_update(void) NONBANKED {

    //if(IS_FRAME_ODD){return;}
    ...
    while (actor) {
        ...
        // SO THIS WORKS (INT16)VM_GLOBAL(9); 9 BEING THE NUMBER IN GLOBALS.H
        //if(IS_FRAME_ODD){goto j_sprites;}
        if(vm_lock_state != 0){goto skip_native;}
        switch (actor->behaviour){

            case 1: //test
                INT16 *mov = &script_memory[ actor->varref[0] ];//(INT16*)actor->varref;
                actor->pos.x += *mov;
                if( (actor->pos.x >> 4) > 140 ){
                    script_memory[actor->varref[0] ] = -(*mov);  //omg finally
                }
                break;

            case 2: // bounce around the screen, no gravity
                actor_move_bounce_x(actor);
                //actor_apply_gravity(actor);
                actor_move_bounce_y(actor);
                break;

            case 3: // walker
                actor_move_bounce_x(actor);
                actor_apply_gravity(actor);
                actor_move_stop_y(actor);
                break;

            case 4: // jump when hitting the ground
                actor_move_bounce_x(actor);
                actor_apply_gravity(actor);
                actor_move_jump_y(actor);
                break;
